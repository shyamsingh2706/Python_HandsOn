...
class LRUCache_OrderedDict:
    ...
    def __init__(self, capacity: int):
        self.myCache = collections.OrderedDict()
        self.space = capacity

    def get(self, key: int) -> int:
        if key not in self.myCache:
            return -1
        self.myCache.move_to_end(key, last=False) # Move recently used item to the left
    ...
    def put(self, key: int, value: int) -> None:
        if key not in self.myCache:
            if self.space > 0:
                ...
                self.myCache.popitem(last=True) # Discard the oldest item at the right end
        self.myCache[key] = value
        self.myCache.move_to_end(key, last=False) # Move recently used item to the left end
        return
